...
export async function updateUserProfile(userId: string, data: {
  ...
  try {
    ...
    return { success: true, user: { email: user.email, displayName: user.displayName, photoUrl: user.photoUrl } };
  } catch (error) {
    console.error('Profile update error:', error);
    return {
      success: false,
      errors: [{ field: 'general', message: 'Profile update failed. Please try again.' }],
    };
  }
}
...
export async function updateUserPreferences(userId: string, preferences: {
  ...
  try {
    ...
    return {
      success: false,
      errors: [{ field: 'general', message: 'Preferences update failed. Please try again.' }],
    };
  }
}
